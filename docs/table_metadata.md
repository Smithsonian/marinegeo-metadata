...
3. `numeric-ranges`: One row with warn/fail minimum and maximum values for a given categorical column in a given data table. Additionally, `range_type` indicates whether the range should be considered inclusive or exclusive of the minimum/maximum values. One CSV per protocol.
